...
def print_tetris(frame, current_config: Config):
    ...
    for i in range(len(tetris_pieces)):
        piece, (x, y), rotation, is_dropping, frames_since_last_movement, character = tetris_pieces[i]
        ...
        tetris_pieces[i] = (piece, (x, y), rotation, is_dropping, frames_since_last_movement, character)
    ...
    if random.random() < current_config.probability_of_new_tetris_piece:
        new_piece = get_random_piece()  # Use a wider range of tetris pieces
        new_x = random.randint(0, width - 1)
        tetris_pieces.append((new_piece, (new_x, 0), 0, False, 0, get_random_char()))


    # Add falling pieces to the frame
    for piece, (x, y), rotation, is_dropping, frames_since_last_movement, character in tetris_pieces:
        if 0 <= y < height and 0 <= x < width:
            draw_piece(frame, piece, x, y, rotation, current_config.use_colors, get_random_char())
...
def draw_piece(frame, piece, x, y, rotation, use_colors, character):
    # Rotate the piece
        ...
        for j in range(len(piece[i])):
            if piece[i][j] == "X":
                if 0 <= y + i < len(frame) and 0 <= x + j < len(frame[0]):
                    if use_colors:
                        frame[y + i] = frame[y + i][:x + j] + "\033[1;31m" + character + "\033[0m" + frame[y + i][x + j + 1:]
                    else:
                        frame[y + i] = frame[y + i][:x + j] + character + frame[y + i][x + j + 1:]
